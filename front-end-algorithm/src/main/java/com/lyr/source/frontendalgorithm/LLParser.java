 ...
 * <p>
 * TODO 如果不基于GrammarNode的结构，先转化成简单的产生式再计算，应该会更简单一些。抽空可能会再实现一个版本。
 *
 * @Author LinYuRong
 * @Date 2021/1/6 14:16
 * @Version 1.0
...
public class LLParser {
    ...
    public static ASTNode parse(String script, GrammarNode grammar) {
        ...
        // 计算First集合
        Map<GrammarNode, Set<String>> firstSets = FirstFollowSet.calcFirstSets(grammar);
        System.out.println("\nFIRST:");
        FirstFollowSet.dumpFirstFollowSets(firstSets);

        // 计算Follow集合
        Map<GrammarNode, Set<String>> followSets = FirstFollowSet.calcFollowSets(grammar, firstSets);
        System.out.println("\nFOLLOW:");
