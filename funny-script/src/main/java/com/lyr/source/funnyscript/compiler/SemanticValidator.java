 ...
 * @Version 1.0
 */
public class SemanticValidator extends FunnyScriptBaseListener {
        ...
        if (ctx.typeTypeOrVoid() != null) {
            if (!hasReturnStatement(ctx)) {
                ...
                if (!(returnType == VoidType.instance())) {
                    at.log("return statement expected in function", ctx);
                }
            }
        }
    }
    ...
    private boolean hasReturnStatement(ParseTree ctx) {
        ...
        for (int i = 0; i < ctx.getChildCount(); i++) {
            ...
            if (child instanceof FunnyScriptParser.StatementContext &&
                    ((FunnyScriptParser.StatementContext) child).RETURN() != null) {
                rtn = true;
                break;
            } else if (!(child instanceof FunnyScriptParser.FunctionDeclarationContext ||
                    child instanceof FunnyScriptParser.ClassDeclarationContext)) {
    ...
    private boolean checkBreak(RuleContext ctx) {
        if (ctx.parent instanceof FunnyScriptParser.StatementContext &&
                (((FunnyScriptParser.StatementContext) ctx.parent).FOR() != null ||
                        ((FunnyScriptParser.StatementContext) ctx.parent).WHILE() != null) ||
                ctx.parent instanceof FunnyScriptParser.SwitchBlockStatementGroupContext) {
            return true;
        } else if (ctx.parent == null || ctx.parent instanceof FunnyScriptParser.FunctionDeclarationContext) {
            return false;
        } else {
